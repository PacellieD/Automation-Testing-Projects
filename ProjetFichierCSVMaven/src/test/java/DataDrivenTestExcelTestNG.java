...
public class DataDrivenTestExcelTestNG {
    ...
    public Iterator<Object[]> preparerDataProvider() {
        ...
        try (FileInputStream fileInputStream = new FileInputStream("src/test/resources/dataSaisieTemps.xlsx")) {
            Workbook workbook = new XSSFWorkbook(fileInputStream);
            Sheet sheet = workbook.getSheetAt(0); // la première feuille du classeur
            Iterator<Row> rowIterator = sheet.iterator();
            if (rowIterator.hasNext()) { // Pour ignorer la première ligne des en-têtes
                rowIterator.next();
            }
            while (rowIterator.hasNext()) {
                Row row = rowIterator.next();
                Iterator<Cell> cellIterator = row.cellIterator();
                String projet = cellIterator.next().getStringCellValue();
                String demande = String.valueOf(cellIterator.next().getNumericCellValue());
                String date = cellIterator.next().getStringCellValue();
                String nbHeures = String.valueOf(cellIterator.next().getNumericCellValue());
                String commentaire = cellIterator.next().getStringCellValue();
                String activite = cellIterator.next().getStringCellValue();
                data.add(new Object[]{projet, demande, date, nbHeures, commentaire, activite});
            }
            workbook.close();
        } catch (Exception e) {
            e.printStackTrace();
